...
async function approve(){if(!chosen)throw new Error('Selecione uma IA.');var p={modelId:chosen.modelId,name:chosen.name,description:chosen.description,tasks:chosen.tasks,provider:chosen.provider,adapter:adapter.value,appEnabled:enableApp.checked,appSlot:slot.value,settings:{width:width.value,height:height.value,steps:steps.value,guidance:guidance.value,strength:strength.value},lastTest:lastTest};var b=await api('/laboratorio-ia/biblioteca',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(p)});await library();note('Salvo na biblioteca: '+b.entry.name,'ok')}
 document.getElementById('saveKey').onclick=function(){sessionStorage.setItem('movimovelLabKey',key.value);note('Senha guardada nesta aba.','ok')};key.value=sessionStorage.getItem('movimovelLabKey')||'';document.getElementById('refresh').onclick=function(){catalog(true).catch(function(e){note(e.message,'err')})};document.getElementById('loadCatalog').onclick=function(){catalog(false).catch(function(e){note(e.message,'err')})};document.getElementById('test').onclick=function(){note('Gerando teste real…','ok');test().catch(function(e){note(e.message,'err')})};document.getElementById('approve').onclick=function(){approve().catch(function(e){note(e.message,'err')})};library().catch(function(e){note(e.message,'err')});
})();</script></body></html>`
}
